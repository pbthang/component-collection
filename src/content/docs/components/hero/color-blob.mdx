...
import ColorBlob from "@/components/hero/ColorBlob.astro";
...
import fullCode from "@/components/hero/ColorBlob.astro?raw";

<Code code={fullCode} lang="html" title="index.html" collapse={"11-20"} />
...
3.  Visit [Clippy](https://bennettfeely.com/clippy/) or any equivalent tool to generate a clip-path polygon. For example, `polygon(61% 100%, 92% 100%, 100% 0, 0 74%, 20% 0)`. Update the blob shape with the clip-path:
    ...
    ```html ins="[clip-path:polygon(61%_100%,92%_100%,100%_0,0_74%,20%_0)]" showLineNumbers=false wrap
    <div
    ...
    <Aside>
      Please note that the clip-path syntax is slightly different in
      TailwindCSS. Replace the spaces with underscores `_` in the clip-path
      values.
    </Aside>

4.  Lower the opacity of the blob shape and add a blur effect to the blob wrapper to create a blurry color blob:

    ```html ins="blur-3xl" ins="opacity-60" showLineNumbers=false wrap
